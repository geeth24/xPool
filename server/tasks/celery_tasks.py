...
def source_from_github_task(
    ...
    Flow:
    1. Search GitHub users by query/language/location
    ...
    print(f"[Celery] Query: {search_query}, Language: {language}, Location: {location}")
    ...
    try:
        ...
        # search GitHub users
        print(f"[Celery] Searching GitHub users...")
        github_users = run_async(
            github_client.search_users(
                query=search_query,
                language=language,
                location=location,
        ...
        for idx, gh_user in enumerate(github_users):
            ...
            candidate = Candidate(
                ...
                profile_url=f"https://x.com/{x_username}" if x_username else gh_profile.get("github_url"),
                followers_count=x_data.get("public_metrics", {}).get("followers_count", 0) if x_data else gh_profile.get("followers", 0),
                following_count=x_data.get("public_metrics", {}).get("following_count", 0) if x_data else gh_profile.get("following", 0),
    ...
    try:
        ...
        for jc in job_candidates:
            ...
            try:
                ...
                print(f"[Celery] tweet_analysis type: {type(candidate.tweet_analysis)}")
                print(f"[Celery] candidate_data tweet_analysis: {type(candidate_data.get('tweet_analysis'))}")
                
                evidence = run_async(
                    grok_client.generate_evidence_card(candidate_data, job_data)
                )
